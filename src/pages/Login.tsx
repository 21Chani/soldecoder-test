...
import { viemClient } from '../lib/viem.config';
...
function Login() {
    ...
    const [loadinBalance, setLoadingBalance] = useState(false);

    async function showUserBalance() {
        const MOCKED_WALLET_ADDRESS =
            '0x248Dd3836E2A8B56279C04addC2D11F3c2497836';

        setLoadingBalance(true);
        try {
            const balance = await readContract(viemClient, {
                abi: Contracts.SpaceCrown.abi,
                address: Contracts.SpaceCrown.deployments[bsc.id],
                functionName: 'balanceOf',
                args: [MOCKED_WALLET_ADDRESS],
            });
            alert(`Balance is ${formatAmount(+formatUnits(balance, 18))}`);
        } catch (e) {
            alert(
                'An unexpected error occurred while fetching balance. Please try again later.'
            );
        } finally {
                        ...
                        >
                            ...
                            >
                                ...
                                <IonButton
                                    ...
                                    color="medium"
                                    onClick={() => showUserBalance()}
                                    disabled={loadinBalance}
                                >
                                    {loadinBalance ? (
                                        <Fragment>
                                            <IonSpinner className="size-" />
                                            Loading Balance
                                        </Fragment>
                                    ) : (
                                        <Fragment>
